	...
	$('#table')
			.bootstrapTable(
					'filterBy',
					{
						...
						commissionPlanTime : [ commissionPlanYearStr,
								commissionPlanMonthStr ],
						commissionStartTime : [ commissionStartFromStr,
								commissionStartToStr ],
						warrantyEndTime : [ warrantyEndFromYearStr,
								warrantyEndFromMonthStr, warrantyEndToYearStr,
								warrantyEndToMonthStr ]
					},
					{
						'filterAlgorithm' : function(row, filters) {
							...
							var rowCommissionPlanndTime = new Date(
									rowCommissionPlanndTimeStamp);
							if (isNumber(filters.commissionPlanTime[0])
									&& !(rowCommissionPlanndTime != undefined && rowCommissionPlanndTime
											.getFullYear() == Number(filters.commissionPlanTime[0]))) {
								return false;
							}
							if (isNumber(filters.commissionPlanTime[1])
									&& !(rowCommissionPlanndTime != undefined && (rowCommissionPlanndTime
											.getMonth() + 1) == parseInt(filters.commissionPlanTime[1]))) {
								return false;
							}
							...
							var rowWarrantyEndTime = new Date(
									rowWarrantyEndTimeStamp);
							if (isNumber(warrantyEndFromYearStr)
									&& isNumber(warrantyEndFromMonthStr)
									&& !(rowWarrantyEndTime != undefined && rowWarrantyEndTimeStamp >= strToTimeStamp(filters.warrantyEndTime[0]
											+ "-"
											+ filters.warrantyEndTime[1]
											+ "-01"))) {
								return false;
							}
							if (isNumber(warrantyEndToYearStr)
									&& isNumber(warrantyEndToMonthStr)
									&& !(rowWarrantyEndTime != undefined && rowWarrantyEndTimeStamp <= strToTimeStamp(filters.warrantyEndTime[2]
											+ "-"
											+ (parseInt(filters.warrantyEndTime[3]))
											+ "-01"))) {
								return false;
							}
							return true;
...
function simpleDescFormmatter(value) {
	var result;
